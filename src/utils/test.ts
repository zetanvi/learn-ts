...
// !接口
let getName = ({ firstName, lastName, show }: info): string => {
  console.log(`${firstName}${lastName} is ${show ? show : ""}`);
  return `${firstName}${lastName} is ${show ? show : ""}`
}
...
let aaa = { firstName: '44', lastName: '00', oo: 'oo' }
getName(aaa as info)